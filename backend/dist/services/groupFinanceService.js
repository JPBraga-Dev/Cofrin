...
export function createSplits(expenseId, amount, memberIds, splitType, input = []) {
...
export function calculateGroupBalances(group) { const balances = new Map(group.members.map(member => [member.userId, 0])); for (const contribution of group.contributions) {
    if (contribution.status === 'CONFIRMED')
        balances.set(contribution.userId, (balances.get(contribution.userId) ?? 0) + contribution.amount);
...
} return group.members.map(member => ({ userId: member.userId, name: member.name, balance: Number(((balances.get(member.userId) ?? 0)).toFixed(2)) })); }
export function groupFund(group) { const raised = group.contributions.filter(c => c.status === 'CONFIRMED').reduce((total, c) => total + c.amount, 0); const paidFromFund = group.expenses.filter(e => e.paymentSource === 'GROUP_FUND').reduce((total, e) => total + e.amount, 0); return raised - paidFromFund; }
