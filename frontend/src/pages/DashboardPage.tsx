...
function SummaryReveal({ children, delay, className = "" }: { children: ReactNode; delay: number; className?: string }) {
  return <motion.div className={`summary-reveal ${className}`} initial={{ opacity: 0, y: 5 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: motionTokens.normal, delay, ease: motionTokens.ease }}>{children}</motion.div>;
}
...
export function DashboardPage() {
  ...
  const committedNeedsAttention = obligations.some((item) => item.status === "OVERDUE");
  ...
  return <div className="dashboard-page">
    <motion.div className="dashboard-header-block" initial={{ opacity: 0, y: 4 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: motionTokens.normal, ease: motionTokens.ease }}>
    ...
    <section className="dashboard-financial" aria-label="Resumo financeiro">
      <SummaryReveal delay={0.03} className="balance-summary-reveal"><Card className="balance-card dashboard-balance"><div className="summary-label balance-kicker"><Wallet size={19} strokeWidth={1.8} /><span>Saldo disponível</span></div><MoneyValue amount={availableBalance} hidden={hidden} size="large" /><div className="balance-footer"><span>{accounts.length} conta{accounts.length === 1 ? "" : "s"} disponível{accounts.length === 1 ? "" : "is"}</span><i aria-hidden="true">·</i><span>Saldo transacional atual</span></div></Card></SummaryReveal>
      <div className="financial-stats">
        <SummaryReveal delay={0.06}><Card className="stat-card income-stat"><div className="summary-label stat-label"><span>Receitas</span><ArrowUpRight size={19} strokeWidth={1.8} /></div><MoneyValue amount={income} type="income" hidden={hidden} /><small>{incomeCount} {incomeCount === 1 ? "entrada" : "entradas"} no período</small></Card></SummaryReveal>
        <SummaryReveal delay={0.075}><Card className="stat-card expense-stat"><div className="summary-label stat-label"><span>Despesas</span><ArrowDownRight size={19} strokeWidth={1.8} /></div><MoneyValue amount={expenses} type="expense" hidden={hidden} /><small>{expenseCount} {expenseCount === 1 ? "saída" : "saídas"} no período</small></Card></SummaryReveal>
        <SummaryReveal delay={0.09} className="committed-summary-reveal"><Card className={`stat-card committed-card ${committedNeedsAttention ? "attention" : ""}`}><div className="summary-label stat-label"><span>Comprometido</span><CalendarClock size={19} strokeWidth={1.8} /></div><MoneyValue amount={committed} hidden={hidden} /><small>Contas pendentes ou atrasadas neste mês.</small></Card></SummaryReveal>
      </div>
    </section>
    ...
    <DashboardSection index={4} className="dashboard-duo">
